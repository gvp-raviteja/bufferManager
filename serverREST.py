...
def categories():
    ...
    for cat in getCategory.keys():
        msg += '<a href="http://10.3.9.231:8081/forums/'+str(cat)+'">'+getCategory[cat]+'</a><br/>'
    return msg
...
def showQues(cat,frm,next):
    ...
    msg = ['<h2>Questions in '+getCategory[cat]+' : '+frm+' are :</h2><br/>']
...
def showAns(cat,frm,qno,next):
    ...
    try:
        ...
        return 'Give valid page number'
    msg = ['<h2>Answers in '+getCategory[cat]+' > '+frm+'>'+str(qno)+' are :</h2><br/>']
...
@route('/<user>/addforum/<catid>/<fname>')
def add_forum(user,catid,fname):
    try:
        catid = int(catid)
        if catid not in range(1,6):
            raise ValueError
...
@route('/<user>/postquestion/<catid>/<fname>',method='POST')
def postQuestion(user,catid,fname):
    try:
        cat = int(catid)
        if cat not in range(1,6):
            raise ValueError
...
@route('/<user>/postanswer/<catid>/<fname>/<qid>',method='POST')
def postAnswer(user,catid,fname,qid):
    try:
        catid = int(catid)
        qid = int(qid)
        if catid not in range(1,6):
...
run(host='10.3.9.231', port=8081, debug=True, reloader=True)
print 'done'